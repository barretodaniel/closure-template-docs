---
path: "/docs/reference/print-directives"
---

# Print Directives

Print directives are post-processing on the output of a [`print` command](print).

Here are all print directives that are available by default. For information on
writing custom print directives, see [Plugins](../developer-guide/plugins).
...
[content kinds](../developer-guide/security#content_kinds) (such as `text`) into `html`.
...
parameter: `false` to truncate without an ellipsis. For example, `{'Lorem Ipsum' │truncate:8}` produces `Lorem...`, while `{'Lorem Ipsum' │truncate:8,false}`
produces `Lorem Ip`.
...
[formatNum()](functions#formatNum) for details.
