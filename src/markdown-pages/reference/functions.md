...
Soy functions are called from within Soy [expressions](expressions).
...
[Bidi Support](../developer-guide/localization#bidi_functions). For information on writing
custom Soy functions, see [Plugins](../developer-guide/plugins#function_plugins).
...
[string literal](expressions#string-literal) that contains some pseudo Soy
...
[string literal](expressions#string-literal) that contains some JS identifier
reference.
...
The keys of a [legacy object map](types#legacy_object_map) as a list. There
...
Returns the substring of `str` beginning at index `start`, and ending at `end - 1`.
...
used in [indexed for loops](control-flow#for-indexed). If the `step` argument
is omitted, it defaults to 1. If the `start` argument is omitted, it defaults to

0. The full form returns a list of plain integers `[start, start + step, start + 2 * step, ...]`.
...
all attribute values are removed so `<a href="https://example.com">click here</a>` is converted to `click here` which is pretty useless.
...
The `msgWithId` function returns a simple [`record`](types#record) consisting
of 2 fields:
...
IMPORTANT: In order to use this function, you _must_ associate the message with
a `let` variable and it _must_ be the only child of that `let` variable.
